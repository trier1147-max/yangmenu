...
async function testAddShareBonus() {
  try {
    const result = await addShareBonus();
...
async function testRecognizeMenuDishFields() {
  try {
    const records = await getRecentRecords(1);
    if (records.length === 0) {
      ok("菜品字段完整性（无历史记录跳过）", true, "跳过");
      return;
    }
    const record = await getRecordById(records[0]._id || records[0].recordId);
    ...
    if (dish.detail) {
      ...
      ok(
        "detail 有 description 或 introduction",
        typeof dish.detail.description === "string" || typeof dish.detail.introduction === "string"
      );
    }
  } catch (e) {
...
async function testRecommendations() {
  try {
    ...
    if (record.recommendations && record.recommendations.length > 0) {
      ...
      ok(
        "recommendation dishIndex 在范围内",
        rec.dishIndex >= 0 && rec.dishIndex < (record.dishes?.length || 999)
      );
    } else {
...
async function testHistoryDelete() {
  try {
    const result = await deleteRecordById("non-existent-id-test-12345");
    ok("deleteRecordById 无效ID不崩溃", true);
  } catch (e) {
    ok("deleteRecordById 函数存在", typeof deleteRecordById === "function");
  }
}
